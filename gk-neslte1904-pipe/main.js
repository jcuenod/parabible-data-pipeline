...
const cols = new Set(
	[].concat(...Object.keys(parse).map((k) => Object.keys(parse[k])))
)
cols.delete("case")
cols.add("case_")
nestle1904.forEach(
	(
		...
		const [_, leader, text, trailingPunc] = punctuatedWord.match(
			punctuationMatch
		)
		...
		if ("gender" in parsing) {
			if (parsing["gender"] === "m") {
				parsing["gender"] = "masc"
			} else if (parsing["gender"] === "f") {
				parsing["gender"] = "fem"
			} else if (parsing["gender"] === "n") {
				parsing["gender"] = "neut"
			}
		}
		const word = {
